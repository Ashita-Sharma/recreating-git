""" git is a CLI application so we use argparse module to parse the CL arguments """

import argparse
import configparser # git config files use microsoft's INI file format
from datetime import datetime

"""To recreate/support .gitignore, we need to be able to identify certain filenames and compare them against patterns"""
from fnmatch import fnmatch

""" git uses SHA-1 extensively. Hence, the hashlib."""
import hashlib
...
argparser = argparse.ArgumentParser(description="Argpaser? I hardly know her!")

argsubparsers = argparser.add_subparsers(title="Commands", dest="command")
argsubparsers.required = True # marks that stuff like "init" "commit" etc are required in when using
"""dest=command just means what we on the cli will be returned to us in a string named [command]"""
...
"""A git repo contains a 'worktree' where all the relevant files are kept, as well as a smaller directory
named git that contains the git configuration files"""
...
"""repositoryformatversion = 0	i.e. the original Git repository format.
filemode = false  only track file contents.(unix tracks file permissions as well)
bare = false  i.e. this repository has a working directory with editable files. true means it's just a Git database, typically used on servers."""
...
def tag_create(repo, name, ref, create_tag_object=False):
    ...
    if create_tag_object:
        ...
        tag.kvlm[b'tagger'] = b'delta <delta@example.com>'
        # …and a tag message!
        tag.kvlm[None] = b"A tag generated by delta, which won't let you customize the message!\n"
...
def index_read(repo):
    ...
    assert version == 2, "delta only supports index file version 2"
